...
export async function POST(req: NextRequest) {
  try {
    ...
    const existing = await listMaterialTypes(storeId);
    if (existing.some((item) => item.name.trim().toLocaleLowerCase("ar") === name.toLocaleLowerCase("ar"))) {
      return NextResponse.json({ error: "نوع المادة موجود بالفعل" }, { status: 409 });
    }
    const type = await createMaterialType(storeId, name, code ?? `TYPE_${randomBytes(8).toString("hex").toUpperCase()}`);
...
function handleError(err: unknown) {
  ...
  if (err instanceof Prisma.PrismaClientKnownRequestError && err.code === "P2002") {
    return NextResponse.json({ error: "نوع المادة موجود بالفعل، جرّب اسمًا آخر" }, { status: 409 });
  }
  console.error(err);
