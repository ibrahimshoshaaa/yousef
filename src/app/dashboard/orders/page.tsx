  ...
  return (
    <main className="mx-auto max-w-7xl space-y-6 px-4 py-6 sm:px-8 sm:py-9">
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div><p className="text-sm font-semibold text-[#96723c]">المبيعات</p><h1 className="mt-1 text-3xl font-bold tracking-tight">الطلبات</h1><p className="mt-2 text-sm text-slate-500">متابعة الطلبات والبنود وحالة الاستهلاك من المتجر المرتبط.</p></div>
        <span className="rounded-full border border-slate-200 bg-white px-4 py-2 text-sm text-slate-600">{count} طلب</span>
      </header>
          ...
          {orders.map(order => (
            <article key={order.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                  <div className="flex flex-wrap items-center gap-2"><h2 className="font-bold">طلب #{order.orderNumber ?? order.id.slice(-8)}</h2><span className="rounded-full bg-emerald-50 px-2.5 py-1 text-xs text-emerald-800">{financialLabels[order.financialStatus ?? ""] ?? order.financialStatus ?? "الحالة غير محددة"}</span></div>
                  <p className="mt-2 text-xs text-slate-500">{new Intl.DateTimeFormat("ar-EG", { dateStyle: "medium", timeStyle: "short", timeZone: "Africa/Cairo" }).format(order.occurredAt)} · {fulfillmentLabels[order.fulfillmentStatus ?? ""] ?? order.fulfillmentStatus ?? "حالة الشحن غير محددة"}</p>
                </div>
                <div className="text-left"><strong className="text-lg">{Number(order.total ?? 0).toFixed(2)} {order.currency}</strong><p className="text-xs text-slate-500">صافي: {Number(order.netSales ?? 0).toFixed(2)} {order.currency}</p></div>
              </div>
      ...
      ) : <div className="rounded-2xl border border-dashed border-slate-300 bg-white px-6 py-16 text-center"><div className="text-4xl text-[#c4a265]">◫</div><h2 className="mt-3 text-xl font-semibold">{q ? "لا توجد نتائج لهذا الرقم" : "لا توجد طلبات بعد"}</h2><p className="mx-auto mt-2 max-w-lg text-sm leading-7 text-slate-500">{q ? "جرّب رقم طلب آخر أو امسح البحث." : "الطلبات ستظهر هنا عند ربط Shopify ومزامنتها. إدخال الطلبات يدويًا غير متاح حاليًا."}</p>{q && <Link className="mt-5 inline-block text-sm font-semibold text-[#315b4c] underline" href="/dashboard/orders">عرض كل الطلبات</Link>}</div>}
