    ...
    <div className="grid gap-3 sm:grid-cols-2">{groups.map((group) => {
      ...
      return <section key={group.key} className="min-w-0 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="divide-y divide-slate-100 px-4">{rows.map((balance) => {
          ...
          return <Link key={balance.id} href={`/dashboard/materials/${balance.materialId}`} className="flex min-w-0 items-center justify-between gap-3 py-3 text-sm hover:text-[#315b4c]"><span className="min-w-0 break-words font-medium">{balance.material.name}</span><span className={`shrink-0 rounded-lg px-2 py-1 text-xs font-bold sm:text-sm ${low ? "bg-amber-50 text-amber-900" : "bg-[#f1f5f2] text-[#263b35]"}`}>{format(Number(balance.quantity))} {balance.material.unit}</span></Link>;
